<#
.SYNOPSIS
  run-validation.ps1 — fan the artefact-bundle migrator across a list of source models,
  migrating each from a SRC server to a DST server via the migrator `remote` command
...
.DESCRIPTION
  ...
  Server URLs + the DST project/model are configured via env or parameters:
    ...
    PARALLEL     / -Parallel    max concurrent migrator runs (default 4)
...
$csproj = Join-Path $scriptDir 'Speckle.Sdk.BundleMigrator.csproj'
...
Write-Host 'Building migrator (Release) ...'
...
$runOne = {
  ...
  if ($legacyApi) { $migratorArgs += '--legacy-api' }

  & dotnet run -c Release --no-build --project $csproj -- $migratorArgs `
