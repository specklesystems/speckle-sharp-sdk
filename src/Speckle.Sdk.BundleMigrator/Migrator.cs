...
using Speckle.Sdk.BundleMigrator.Transports;
...
/// <summary>
/// End-to-end artefact-bundle migrator: resolves an object graph (a DuckDB packfile, on disk or fetched
...
/// <see cref="MigratorCommandLine"/> command actions.
/// </summary>
...
)
{
  ...
  // Everything the migrator writes lives under one root it owns, so it can be emptied without touching
  // anything else in %TEMP%.
  private static string WorkRoot => Path.Combine(Path.GetTempPath(), "speckle-bundle-migrator");
  ...
  )
  {
    ...
    http.DefaultRequestHeaders.UserAgent.ParseAdd("speckle-bundle-migrator/1.0");
