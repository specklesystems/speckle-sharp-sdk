...
public sealed class SpeckleObjectDeserializer2
{
  ...
  private async ValueTask<object?> DeserializeJsonAsyncInternal(string objectJson)
  {
    ...
    using JsonReader reader = Pool.GetJsonTextReader(new StringReader(objectJson));
  ...
  {
    reader.Read();
    Dictionary<string, object?> dict = Pool.DictPool.Get();
    ...
    var b = Dict2Base(dict);
    Pool.DictPool.Return(dict);
    return b;
  }
