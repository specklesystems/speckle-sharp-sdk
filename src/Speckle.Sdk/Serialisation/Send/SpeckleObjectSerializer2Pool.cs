...
public class SpeckleObjectSerializer2Pool
{
  ...
  public  JsonTextWriter GetJsonTextWriter(Stream stream) => new (new StreamWriter(stream))
  {
    ArrayPool = _charPool
  };
  
  public  JsonTextWriter GetJsonTextWriter(TextWriter writer) => new (writer)
  {
    ArrayPool = _charPool
  };
  
  private  readonly RecyclableMemoryStreamManager _recyclableMemoryStreamManager = new(new RecyclableMemoryStreamManager.Options()
  {
