...
public abstract class ChannelLoader<T>
{
  ...
  protected async Task GetAndCache(IEnumerable<string> allChildrenIds, CancellationToken cancellationToken, int? maxParallelism = null) =>
    await _channel.Source(allChildrenIds, cancellationToken)
      ...
      .PipeAsync(maxParallelism ??MAX_PARALLELISM_HTTP, async x => await Download(x).ConfigureAwait(false), -1, false, cancellationToken)
      .Join()
