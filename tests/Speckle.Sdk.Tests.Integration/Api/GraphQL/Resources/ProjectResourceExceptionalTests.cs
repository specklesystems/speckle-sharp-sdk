...
public class ProjectResourceExceptionalTests : IAsyncLifetime
{
  ...
  public async Task ProjectUpdateRole_NonExistentProject(string? newRole)
  {
    ...
    ex.InnerExceptions.Single().Should().BeOfType<SpeckleGraphQLException>(); //v3 server responds with SpeckleGraphQLStreamNotFoundException exception, v2 reponds with SpeckleGraphQLForbiddenException
  }
